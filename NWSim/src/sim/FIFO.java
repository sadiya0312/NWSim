...
public class FIFO {
	...
	public void FIFO_red() throws InterruptedException{
		...
		for(int map=0;map<Framework.Mapper;map++){
			for(int queue=0;queue<Framework.queuesinmapper;queue++){
				...
				int qsize=(Graph.all_mappers[map].mapper_queue.size());          	

				for(int length=0;length<=(qsize+1);length++){

					if((Graph.all_mappers[map].mapper_queue.get(queue).peek())!=null){
						coflowid=Graph.all_mappers[map].mapper_queue.get(queue).poll();
